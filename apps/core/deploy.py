...
def fetch_node_health(node: Node, timeout: int = 5) -> dict | None:
    """Опрашивает агент ноды на /health. Возвращает словарь статуса или None при ошибке."""
    url = f"https://{node.address}:{node.port_agent}/health"
    headers = {"Authorization": f"Bearer {node.agent_token}"}
    try:
...
def fetch_node_metrics(node: Node, timeout: int = 5) -> dict | None:
    """Опрашивает агент ноды на /metrics. Возвращает словарь метрик или None при ошибке."""
    url = f"https://{node.address}:{node.port_agent}/metrics"
    headers = {"Authorization": f"Bearer {node.agent_token}"}
    try:
        response = requests.get(url, headers=headers, timeout=timeout)
        response.raise_for_status()
